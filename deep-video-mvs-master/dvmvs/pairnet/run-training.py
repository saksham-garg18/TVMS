...
# Workarounds for cuDNN errors with newer GPUs
# Disable cuDNN to avoid compatibility issues between CUDA 10.2 and RTX 4050
...
class TrainingHyperparameters:
    ...
    batch_size = 2  # Reduced from 4 to avoid GPU memory issues with 6GB VRAM
    learning_rate = 1e-4
    momentum = 0.9
    beta = 0.999
    weight_decay = 0
...
def main():
    ...
    print("Loading pre-trained component weights for fine-tuning...")
    
    # Path to the FOLDER containing the downloaded component files (0_..., 1_..., etc.)
    # MAKE SURE YOU UPDATE THIS PATH TO YOUR ACTUAL PATH
    weights_dir = r'/mnt/d/TVMS/deep-video-mvs-master/dvmvs/pairnet/weights' 
    
    # Get a sorted list of the weight files.
    # The names are '0_feature_extractor', '1_feature_pyramid', '2_encoder', '4_decoder'.
    # We need to map them correctly to the model list.
    import glob
    component_files = sorted([Path(f) for f in glob.glob(os.path.join(weights_dir, '*')) if os.path.isfile(f)])
    
    # Define the mapping from the file names to the indices in the `model` list.
    # The `pairnet` model has 4 components.
    # File '0_...' -> model[0] (FeatureExtractor)
    # File '1_...' -> model[1] (FeatureShrinker)
    # File '2_...' -> model[2] (CostVolumeEncoder)
    # File '4_...' -> model[3] (CostVolumeDecoder)
    # Note: The original repo has a file named '4_decoder', not '3_decoder'. We must handle this.
    
    # Find the correct files for each module index
    file_map = {
        0: next((f for f in component_files if f.name.startswith('0_')), None), # feature_extractor
        1: next((f for f in component_files if f.name.startswith('1_')), None), # feature_shrinker
        2: next((f for f in component_files if f.name.startswith('2_')), None), # cost_volume_encoder
        3: next((f for f in component_files if f.name.startswith('3_')), None)  # cost_volume_decoder (note: file is 3_decoder not 4_decoder)
    }

    # Now, load the weights for each module in the list
    for i, module in enumerate(model):
        component_path = file_map.get(i)
        if component_path and component_path.exists():
            print(f"Loading weights for component {i} from: {component_path.name}")
            
            # Load the weights dictionary from the file
            weights = torch.load(component_path)
            
            # Load the state dictionary into the corresponding module
            # Check if weights is a dict with 'state_dict' key, otherwise use it directly
            if isinstance(weights, dict) and 'state_dict' in weights:
                module.load_state_dict(weights['state_dict'])
            else:
                module.load_state_dict(weights)
        else:
            print(f"Warning: Could not find weight file for model component {i}. It will remain randomly initialized.")

    print("Pre-trained PairNet components loaded successfully.")
