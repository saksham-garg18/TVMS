...
def read_split(path):
    scenes_txt = np.loadtxt(path, dtype=str, ndmin=1)
    return scenes_txt
...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, sample_index):
        ...
        # Check if npz files are in a subdirectory
        npz_dir = scene_path / 'training_npz'
        if npz_dir.exists():
            scene_npzs = sorted(npz_dir.files('*.npz'))
        else:
